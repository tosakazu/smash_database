...
* Data is committed **only on its data branch**, and only under `data/startgg/<Region>/`
  ...
  `data/startgg/<Region>/curation/` (edit, commit, push like any data change).
...
└── data/startgg/Japan/         one directory per region: index files + events
    ...
    ├── users_curated.jsonl     generated by curate.py: prefecture resolved from each player's city
    ├── curation/               hand-maintained tables (edit these; nothing regenerates them)
...
| `scripts/common/curate.py` | Writes `curated.json` next to each event (is it a 1-on-1 event, name-based flags such as special rules / private / restricted / lower class / pre-tournament, calendar flags, class-bracket phase groups, venue prefecture) and `users_curated.jsonl` (player prefecture from the free-text city). The rules live in `scripts/<Region>/classify.py` (Japan: `scripts/Japan/classify.py`, `prefecture.py`); `--all` reprocesses everything after a rule change. Consumers (the spsp build) read these files and never re-derive them |

...
4. `scripts/common/derive.py --region Japan` refreshes `derived.json` for events whose files changed and
   rewrites `users_derived.jsonl` when the resolved prefectures change
   (see [data_model.md](data_model.md), "derived.json"). A failure here stops the run: the build refuses

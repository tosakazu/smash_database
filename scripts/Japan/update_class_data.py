...
def detect_unseparated(since_days: int, events_root: Path = None) -> list[str]:
    """直近 since_days 日のイベントで class phase 未マークの event dir を返す."""
    events_root = Path(events_root) if events_root else EVENTS_ROOT
...
def main(argv=None) -> int:
    ...
    from scripts.Japan import (fetch_event_phases, fetch_class_phase_standings, fetch_class_phase_players,
                               build_class_virtual_tournaments)
    dirs = detect_unseparated(args.since_days, events_root)

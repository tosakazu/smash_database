...
DEFAULT_EVENTS_ROOT = None   # derived from --region as data/startgg/<region>/events
...
# Virtual class-bracket events (generated by update_class_data.py from phases.json as
# `.../class_phases/<X>_virtual`) are by design not registered in tournaments.jsonl.
# In fact, of the 27,136 registered events none are class_phases, and all 908 on disk
# are intentionally unregistered. Without excluding them here, real unregistered events would be buried.
...
def main() -> int:
    ...
    if not tournaments_file.is_file():
        print(f"{tournaments_file} not found.", file=sys.stderr)
        return 1
    if not events_root.is_dir():
        print(f"{events_root} not found.", file=sys.stderr)
        return 1
    ...
    for event_dir in iter_event_dirs(events_root):
        ...
        if attr is None:
            missing_events.append(
                MissingEvent(
                    ...
                    reason="attr.json could not be read",
                )
            )
            continue
        ...
        reason = "path not registered in tournaments.jsonl"
        missing_events.append(
            MissingEvent(
                path=event_dir,
    ...
    print("Found unregistered events:")
    ...
    if not args.apply:
        print(" Pass --apply to update tournaments.jsonl.")
        return 1
    ...
    for item in missing_events:
        if item.event_id is None:
            print(f"[SKIP] {item.path}: cannot add because event_id is unavailable.", file=sys.stderr)
            continue
        try:
        ...
        if tournament_id is None:
            print(f"[SKIP] {item.path}: could not fetch the tournament ID.", file=sys.stderr)
            continue
        ...
        if tournament_entry:
            ...
            if any(ev.get("event_id") == item.event_id or ev.get("path") == rel_path for ev in events):
                print(f"[SKIP] {rel_path}: already registered.")
                continue
            events.append(event_payload)
            print(f"[ADD] Added event {item.event_id} to existing tournament {tournament_id}.")
        else:
            ...
            print(f"[ADD] Added new tournament {tournament_id} ({name}).")
        updated = True

    if not updated:
        print("No changes to tournaments.jsonl.")
        return 1

    if args.dry_run:
        print("Dry-run mode; tournaments.jsonl not rewritten.")
        return 0

    write_tournaments(tournaments, tournaments_file, args.indent)
    print(f"Updated {tournaments_file}.")
    return 0

...
# class bracket の仮想イベント (update_class_data.py が phases.json から生成する
# `.../class_phases/<X>_virtual`) は、設計上 tournaments.jsonl に登録しない。
# 実際 27,136 件の登録 event のうち class_phases は 0 件で、ディスク上の 908 件は
# すべて意図的に未登録。ここで除外しないと本当の未登録 (実イベント) が埋もれる。
CLASS_VIRTUAL_PARENT = "class_phases"


def is_class_virtual(path: Path) -> bool:
    return CLASS_VIRTUAL_PARENT in path.parts


def iter_event_dirs(events_root: Path) -> List[Path]:
    return [path.parent for path in events_root.glob("**/attr.json")
            if not is_class_virtual(path.parent)]
